...
const Container = styled.div`
  padding: 2rem;
  max-width: 700px;
  margin: 12rem auto 60px auto; /* Added bottom margin */
  color: #fff;
...
const Curriculum = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/Curriculum")
      .then(res => res.json())
      .then(data => setCurriculumData(data))
      .catch(err => console.error("Error fetching curriculum data:", err));
  }, []);
  ...
  return (
    <>
      ...
      <ContentWrapper>
        ...
        <Container>
          <Header>
            <Title>Curriculum & Syllabus</Title>
            <Subtitle>
              Download the latest syllabus for each year of the ECE branch.
            </Subtitle>
          </Header>
          <YearGrid>
            {curriculumData.map(({ year,pdfUrl, description }) => (
              <YearCard key={year}>
                <YearTitle>{year}</YearTitle>
                <YearDesc>{description}</YearDesc>
